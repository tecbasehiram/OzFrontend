...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    ...
    window.recarregarEmailListItems = () => {
      selectors.emailListItems = Array.from(document.querySelectorAll('.email-list-item'));
      window.mostrarPaginacaoAtual();
    };

    window.mostrarPaginacaoAtual = () => {
      const atual = window.currentFolder?.toLowerCase(); // Ex: 'sent', 'spam', 'drafts'

      if (!atual) return;

      const spans = document.querySelectorAll('#pagination span.email-count');

      spans.forEach(span => {
        // Checa se o span tem o atributo correspondente à caixa atual
        if (span.dataset[atual] !== undefined) {
    ...
    window.recarregarEmailBoxListItems = () => {
      selectors.emailFilterByFolders = Array.from(document.querySelectorAll('.email-filter-folders li'));

      selectors.emailFilterByFolders.forEach(folder => {
        folder.addEventListener('click', e => {
          const targetFolder = e.currentTarget.getAttribute('data-target');
          document.querySelector(".email-search-input").value = "";

          window.currentFolder = targetFolder;
          console.log(window.currentFolder)
          window.mostrarPaginacaoAtual();
          ...
          selectors.emailListItems.forEach(emailItem => {
            const matchesFolder = emailItem.hasAttribute(`data-${targetFolder}`);
    ...
    selectors.emailCompose.addEventListener('hidden.bs.modal', () => {
      ...
      if(document.getElementById('emailContacts')) {
      ...
      selectors.emailListRead.addEventListener('click', () => {
        selectors.emailListItemInputs.forEach(input => {
          if (input.checked) {
            ...
            icon.classList.replace('bx-envelope-open', 'bx-envelope');
          }
        });
        selectors.selectAllEmails.indeterminate = false;
        selectors.selectAllEmails.checked = false;
      ...
      selectors.emailListItemActions.forEach(action => {
        action.addEventListener('click', e => {
          ...
          if (action.classList.contains('email-delete')) {
            ...
            icon.classList.toggle('bx-envelope-open');
            icon.classList.toggle('bx-envelope');
          }
        });
      });
    }
