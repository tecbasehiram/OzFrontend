...
function gerarJsonSidebar() {
  const data = { navigation: {}, suggestions: {} };
  const layoutMenu = document.querySelector("#layout-menu");

  if (!layoutMenu) return data;

  // Seleciona apenas os itens de menu com links e que NÃO sejam menu-toggle
  const menuItems = layoutMenu.querySelectorAll("li.menu-item > a.menu-link:not(.menu-toggle)");

  menuItems.forEach(link => {
    const icon = link.querySelector("i.menu-icon");
    const name = link.querySelector("div")?.innerText.trim() || "Sem nome";
    const url = link.getAttribute("href") || "#";
    // Identificar a seção mais próxima (menu-header)
    const sectionName = link.closest("ul")?.parentElement.querySelector("div")?.innerText.trim();
    const hasGrandParent = link.closest("ul")?.parentElement.parentElement.parentElement.parentElement.classList.contains("menu-sub");
    const sectionParentName = hasGrandParent ? link.closest("ul")?.parentElement.parentElement.parentElement.parentElement.querySelector("div")?.innerText.trim() : sectionName;

    // Inicializa a seção, se ainda não existir
    if (!data.navigation[sectionName]) {
      data.navigation[sectionName] = [];
    }

    // Adiciona o item ao JSON
    data.navigation[sectionName].push({
      name: `${name} ${sectionName ? `(${sectionName}${hasGrandParent ? ` - ${sectionParentName}` : ""})` : ""}`,
      icon: icon ? [...icon.classList].find(c => c.startsWith("bx-") || c.startsWith("fa-")) || "" : "",
      url
    });
  });

  // Define sugestões fixas (fora do loop)
  data.suggestions = {
    "Pesquisas Populares": [
      {
        "name": "Chat",
      ...
      if (state.isOpen) {
        ...
        if (cancelIcon) {
          cancelIcon.innerHTML =
            '<span class="text-body-secondary w-100"></span> <span class="icon-base icon-md bx bx-x text-heading"></span>';
        }
                      ...
                      ${items.map(
                        item => html`
                          <a href="${item.url}" class="suggestion-item d-flex align-items-center">
                            ${item.icon.startsWith("bx") ? html`<i class="icon-base bx ${item.icon} me-2"></i>` : html`<i class="icon-base fa-solid ${item.icon} me-2"></i>`}
                            <span>${item.name}</span>
                          </a>
                        `
                      )}
              ...
              item({ item, html }) {
                return html`
                  <a href="${item.url}" class="d-flex justify-content-between align-items-center">
                    <span class="item-wrapper"><i class="icon-base ${item.icon.startsWith("bx") ? `bx ${item.icon}` : item.icon.startsWith("fa-") ? `fa-solid ${item.icon != "fa-solid" ? item.icon : "fa-question"}` : "fa-solid fa-question"}"></i>${item.name}</span>
...
if (document.documentElement.querySelector('#autocomplete')) {
    document.addEventListener('sidebarReady', loadSearchData);
}
